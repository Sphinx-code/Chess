...
def nice_info(B,B2,depth): 
    one*(function_val(B2,depth)-function_val(B,depth))
    two*(value_mid_control(B2)-value_mid_control(B))
    ...
    six*(enemy_treat_board_depth(B2,depth)-enemy_treat_board_depth(B,depth))
...
def best_move_piece(piece,pos, B,depth):
    ...
    for new_pos in list_pos:
        if not king_wellness_move(B,[pos,new_pos]) :
            continue
        quality_move = dif_value_tot(pos,new_pos,B,depth)
...
def value_player(B,depth):
    value = 0
    value =  one*function_val(B,depth) + two*value_mid_control(B)     
    value = value + three*value_protect(B)+ four*board_control(B) + five*value_pawn_control(B)
    value = value + six*enemy_treat_board_depth(B,depth)
    return value  
            ...
            if B[i,j] > 0:
                ...
                if direct_att_pos_depth(pos_inv,B2,depth):
                    ...
                    if least_minion != [0,0,0]:                     
                        B3 = move([least_minion[1],least_minion[2]], pos_inv,B2)
                        B4 = invert_board(B3)
                        if direct_att_pos(pos,B4) and B[i,j] != king:
                            val = val-least_val
    return val
